...
# List of vehicle IDs to pull data for
vehicle_ids = [
...
print(f"[{start_time}] Gathering and publishing started...")

# Download and publish breadcrumbs live
    ...
    try:
        with urllib.request.urlopen(url) as response:
            ...
            for record in breadcrumbs:
                ...
                if count % 50000 == 0:
                    print(f"[{datetime.now()}] Published {count} records so far... (Current Vehicle ID: {vehicle_id})")
    except Exception:
        failed += 1
...
print(f"[{end_time}] Publishing complete.")
print(f"Total records published: {count}")
print(f"Total vehicles failed: {failed}")
